...
export default {
  ...
  mounted:function(){
     ...
     var scrolldom = document.getElementById('mescroll3');
     scrolldom.onscroll = function(){
        localStorage.firstposition =scrolldom.scrollTop;                
     }  
  },
  activated(){ 
     var scrolldom = document.getElementById('mescroll3');    
     if(localStorage.firstposition){
        scrolldom.scrollTop=localStorage.firstposition;
     }
  },
  // destoryed(){
  //   self.mescroll = null;
  ...
  methods: {
     infinite(page){          
          let _this = this;
          this.axios.post('/new/newList',{page:page.num}).then(function(res){
